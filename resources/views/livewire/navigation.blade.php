<nav class="bg-gray-800" x-data= "{ open: false }"> {{-- xdata pertenece a ALPINE, esto abre y cierra el menu vista movil--}}
            ...
            <div class="flex space-x-4">
             ...
             {{--  @endcan --}}
              ...
              {{-- @foreach ($rutas as $ruta) --}}
                ...
                <a href="/inicio{{-- {{route('posts.category', $ruta) }} --}}" {{-- class="text-gray-300 hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium" --}}>
                <button
                type="button"
              ...
              {{-- {{$ruta->name}} --}}sadads
                ...
                {{-- @endforeach --}}
                <a  href="/inicio{{-- {{route('posts.refrescar')}} --}}">
                <button
                type="button"
                class="border border-gray-700 bg-gray-700 text-white rounded-md px-4 py-1 {{-- m-2 --}} transition duration-500 ease select-none hover:bg-gray-800 focus:outline-none focus:shadow-outline"
              >
    ...
    <div class="sm:hidden" id="mobile-menu" x-show="open" x-on:click.away=" open = false">
      <div class="space-y-1 px-2 pt-2 pb-3">
        ...
        <a href="{{route('posts.ruta', $ruta) }}" class="text-gray-300 hover:bg-gray-700 hover:text-white block px-3 py-2 rounded-md text-base font-medium">{{$category->name}}</a>
      @endforeach --}}
